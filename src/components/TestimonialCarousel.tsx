...
// Default data with Unsplash images
...
const TestimonialCarousel = ({ testimonials = DEFAULT_TESTIMONIALS }: { testimonials?: Testimonial[] }) => {
  ...
  return (
    /* CHANGED: Removed bg-[#010a0f] and added bg-transparent */
    <section className="relative min-h-[850px] w-full bg-transparent flex flex-col items-center justify-center overflow-hidden py-20 font-sans z-10">
      
      {/* 1. SECTION HEADER (Cyan Accent) */}
      <div className="absolute top-12 md:top-16 flex flex-col items-center z-50">
        <h2 className="text-white text-4xl md:text-6xl font-bold tracking-tight uppercase italic">
          Testimonials
        </h2>
      </div>

      {/* 2. THE PORTAL STAGE */}
      <div className="relative w-full max-w-[1600px] h-[550px] md:h-[650px] flex items-center justify-center mx-auto overflow-hidden">
        ...
        {testimonials.map((item, index) => {
          ...
          return (
            ...
            >
              {/* --- ORBIT RING (Cyan Themed) --- */}
              {/* Added backdrop-blur to the center ring to make it feel like a lens over the halo background */}
              <div
                ...
                w-[340px] h-[340px] md:w-[520px] md:h-[520px] ${
                  isCenter
                    ? "border-[#00d8ff]/40 shadow-[0_0_50px_rgba(0,216,255,0.15)] backdrop-blur-sm"
                    : "border-white/10"
                }`}
              >
                ...
                >
                  ...
                  >
                    <div className={`rounded-full flex items-center justify-center transition-all duration-700
                      ${isCenter 
                        ? "w-12 h-12 md:w-14 md:h-14 bg-[#00d8ff] shadow-[0_0_30px_#00d8ff]" 
                        : "w-8 h-8 bg-white/20"
                      }
                    `}>
                ...
                `}>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    {/* CHANGED: Removed bg-[#010a0f] from avatar container to stay unified */}
                    <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border border-white/10 p-1 bg-white/5 backdrop-blur-md shadow-2xl overflow-hidden">
                      <img
                        src={item.avatar || `https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop`}
                        alt={item.name}
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[#00d8ff] text-[8px] md:text-[10px] font-bold tracking-[0.4em] uppercase opacity-80 mt-1">
                        {item.role}
                      </p>
                    </div>
                  </div>
      ...
      <div className="flex items-center gap-10 z-50 mt-12">
        <button
          onClick={handlePrev}
          ...
          className={`w-14 h-14 md:w-16 md:h-16 rounded-full border flex items-center justify-center transition-all group backdrop-blur-md
            ${currentIndex === 0 
              ...
              : "border-white/10 text-white/40 hover:text-white hover:border-[#00d8ff] hover:bg-white/10"}
          `}
        >
      ...
      {/* CHANGED: Removed local radial gradient DIV to ensure Index halos are unobstructed */}
